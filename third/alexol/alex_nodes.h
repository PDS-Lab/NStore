...
public:
  ...
  std::pair<int, int> insert(const T &key, const P &payload) {
    ...
    // Insert
    std::pair<int, int> positions = find_insert_position(key); // diff 先插入，在判断开销
    ...
    // min_limit_ = std::min((double)key, min_limit_);
    // max_limit_ = std::max((double)key, max_limit_); // zzy : min_limit和Max_limit到底是干啥的，为啥不能换成first_key和last_key
    // 节点没有对该两变量进行修改，在进行expand_root时会造成计算的错误，看看apex怎么做的
  ...
  void resize_from_existing(self_type *old_node, double target_density,
    ...
    if (num_keys_ == 0) { // diff
      initialize(num_keys_, kMinDensity_); // 初始化节点的kv array,bitmap等参数
      expansion_threshold_ = data_capacity_;
      contraction_threshold_ = 0;
      return;
    }
    ...
    /*
    #if ALEX_DATA_NODE_SEP_ARRAYS  // diff 
        key_allocator().deallocate(key_slots_, data_capacity_);
        payload_allocator().deallocate(payload_slots_, data_capacity_);
    #else
        value_allocator().deallocate(data_slots_, data_capacity_);
