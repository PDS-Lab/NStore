...
namespace nstore
{
  ...
  private:
    ...
    struct Worker
    {
      Worker(uint16_t core_id, uint8_t tid, ThreadPool *pool) : tid(tid), pool(pool), stop(false)
      {
        w_ = new W();
        w_->worker_thread = std::thread(&Worker::routine, this, core_id);
      }
      ~Worker()
      {
        {
  ...
  public:
    ThreadPool(uint8_t numa_id = 0, uint8_t threads = 1) : busy_bits(0), rng(time(nullptr))
    {
      workers.reserve(max_thread_count);
      add_threads(numa_id, threads);
    }

    uint8_t count() const { return workers.size(); }

    void add_threads(uint8_t numa_id, uint8_t threads)
    {
      threads = min(threads, max_thread_count - count());
      for (uint8_t i = 0; i < threads; ++i)
        workers.emplace_back(numa_id == 0 ? (32 + i) : (48 + i), workers.size(), this); // FIXME(zzy): 不同cpu需要不同参数
    }
