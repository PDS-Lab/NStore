...
public:
    ...
    template<typename RandomIt, typename key_t = T>
    void init(RandomIt first, size_t size, size_t sg_num, T (*func)(const key_t&) = to_key<T, key_t>) {
        if(stage_2) free(stage_2);
        //  common_alloc->Free(stage_2, nr_stage_2 * sizeof(stage_2_model_t));
        if(stage_1) free(stage_1);
        //  common_alloc->Free(stage_1, sizeof(stage_1_model_t));
